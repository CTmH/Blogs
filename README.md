...
## 设计实现过程
日志中的问题解决部分
- 单元测试见[项目第4周日志](https://ctmh.github.io/Blogs/Week4Log)
- 设计图和文档见上方 面向对象分析
## 性能分析和改进
过程见[项目第4周日志](https://ctmh.github.io/Blogs/Week4Log)
话费时间300分钟左右（含学习使用分析工具的时间）
## 代码评审
采用GitHub的pull request 和 merge 评论确认机制进行代码评审
## 扩展需求
针对题目中提出的不同地图适配问题：在开始的时候可以选择北京或上海地图。模块变化：后端C++中，在程序开始阶段选择北京或者上海，通过输入城市选择地图线路存储文件；在UI中，通过复选框判断选择的城市是北京或者上海，再向可执行文件传参；在文件夹内需要存放对应城市的地铁线路。
